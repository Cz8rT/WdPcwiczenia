...
void printAvgYear(float pointer[][0]){
    float * rok;        // Wskaznik 4 bajtowy na adres poczatku tablicy
    float * miesiac;    // Wskaznik 4 bajtowy na adres poczatku tablicy
    float podsuma, suma;
    int licznik = 0;
...
void printAvgMonth(float pointer[][0]){
    ...
    for (miesiac = *pointer; miesiac <= *pointer + 11; miesiac++){
        for(rok = miesiac, podsuma = 0; rok <= *pointer + 48 + 11; rok = rok + 12){ // Wskaznikowi rok przypisana jest aktualna wartosc wskaznika miesiac
            podsuma += *rok;
        }
